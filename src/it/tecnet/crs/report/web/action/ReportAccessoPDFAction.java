...
public class ReportAccessoPDFAction extends BaseAction implements ModelDriven<ReportPDFBean>{
	...
	public ReportAccessoPDFAction(ReportPDFService reportPDFService) {
		super();
	 ...
	 * *****************************************
	 */
	public String getReportAccessoPDF(){
		...
		try {
			...
			// Chiudo il documento
			document.close();
			getModel().setExportPDFFileName("ReportAccessoATPO.pdf");
	...
	public void addParereMedicoSintesi(Document document, ReportAccessoPDFDto report,ReportAccessoPDFDto nonConf ,Long idFase) throws DocumentException{
		...
		long hack = -1;
		List<ReportAccessoPDFDto> listaVC = reportPDFService.getVarCompByIdMNonConf(report.getIdSSessione(),idFase,idMNonConf);
		List<ReportAccessoPDFDto> listaVCHack = reportPDFService.getVarCompByIdMNonConf(report.getIdSSessione(),idFase,hack);
...
//		presenti pareri su bozza relazione e dissenso/accettazione
	...
	public String getAllegatoReportAccessoPDF(){
		...
		try {
			...
			// Eseguo la query che estrae i dati da visualizzare in stampa
			report = reportPDFService.getAllegatoReportAccessoPDF(idSSessione);
			List<AtpoPratiche> objects = reportPDFService.getReportAllegatiPDF(report.getSede(),report.getIdSessione());
			// Apro il documento
			document.open();
			// Imposto e implemento il pdf
