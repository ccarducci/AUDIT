var DocMancanteTable = function() {

	var initTable1 = function() {
		var table = $('#docMancanteTable');
		...
		var oTable = table
				.dataTable( {
					...
					"sAjaxSource" : "/CruscottoAuditAtpoWebWeb/jsonATPO/getDocMancanteRiepFascTable",
					"processing" : false,
					"serverSide" : true,
					...
					"aoColumns" : [
							{
								...
								"mRender" : function(data, type, row) {
									...
									content += '<input type="checkbox"  name="ids" id="docManc_'
											+ row.idDettFasc
											+ '" class="md-check docManc" value="'
											+ row.idDettFasc + '">';
									content += '<label for="docManc_' + row.idDettFasc + '">';
									content += '<span></span>';
