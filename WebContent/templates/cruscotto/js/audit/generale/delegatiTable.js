var DelegatiTable = function() {

	var initTable1 = function() {
		var table = $('#delegati_table');
		...
		var oTable = table
				.dataTable( {
					...
					"sAjaxSource" : "/CruscottoAuditAtpoWebWeb/json/getDelegati",
					"processing" : false,
					"serverSide" : true,
					...
					"aoColumns" : [
							{
								...
								"mRender" : function(data, type, row) {
									...
									content += '<input type="checkbox"  name="ids" id="chkDelegato'
											+ row.idUtente
											+ '" class="md-check chkDelegato" value="'
											+ row.idUtente + '">';
									content += '<label for="chkDelegato' + row.idUtente + '">';
									content += '<span></span>';
