var DomAttivitaDettaglioDaAssociareTable = function() {

	var initTable1 = function() {
		
		
		var table = $('#addDominiAttivitaDettaglioTable');
		var oTable = table
				.dataTable( {
					...
					"sAjaxSource" : contextApp + "/jsonATPO/searchDominiAttivitaDettaglioDaAssociare",
					"processing" : false,
					"serverSide" : true,
					...
					"aoColumns" : [
							{
								...
								"mRender" : function(data, type, row) {
									...
									content += '<input type="checkbox"  name="ids" id="domADAl_'+row.id+'" class="md-check chkDomAttivitaDettaglioAl" value="'+row.id+'">';
									content += '<label for="domADAl_'+row.id+'">';
									content += '<span></span>';
		...
		var tableWrapper = $('#addDominiAttivitaDettaglioTable_wrapper');
