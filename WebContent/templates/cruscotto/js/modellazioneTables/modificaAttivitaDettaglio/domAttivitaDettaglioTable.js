...
var DomAttivitaDettaglioTable = function() {

	var initTable1 = function() {
		
		
		var table = $('#dominiAttivitaDettaglioTable');
		var oTable = table
				.dataTable( {
					...
					"sAjaxSource" : contextApp + "/jsonATPO/searchDominiAttivitaDettaglio",
					"processing" : false,
					"serverSide" : true,
					...
					"aoColumns" : [
							{
								...
								"mRender" : function(data, type, row) {
									...
									content += '<input type="checkbox"  name="ids" id="domAD_'+row.idAssociazione+'" class="md-check chkDomAttivitaDettaglio" value="'+row.idAssociazione+'">';
									content += '<label for="domAD_'+row.idAssociazione+'">';
									content += '<span></span>';
		...
		var tableWrapper = $('#dominiAttivitaDettaglioTable_wrapper');
