var ComponentiTecniciAttivitaDettaglioDaAssociareTable = function() {

	var initTable1 = function() {
		
		
		var table = $('#addCompTecAttivitaDettaglioTable');
		var oTable = table
				.dataTable( {
					...
					"sAjaxSource" : contextApp + "/jsonATPO/searchCompTecniciAttivitaDettaglioDaAssociare",
					"processing" : false,
					"serverSide" : true,
					...
					"aoColumns" : [
							{
								...
								"mRender" : function(data, type, row) {
									...
									content += '<input type="checkbox"  name="ids" id="compTecADAl_'+row.id+'" class="md-check chkCompTecAttivitaDettaglioAl" value="'+row.id+'">';
									content += '<label for="compTecADAl_'+row.id+'">';
									content += '<span></span>';
		...
		var tableWrapper = $('#addCompTecAttivitaDettaglioTable_wrapper');
