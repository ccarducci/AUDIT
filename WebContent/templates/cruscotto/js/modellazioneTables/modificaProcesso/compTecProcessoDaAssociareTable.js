var ComponentiTecniciProcessoDaAssociareTable = function() {

	var initTable1 = function() {
		
		
		var table = $('#addCompTecProcessoTable');
		var oTable = table
				.dataTable( {
					...
					"sAjaxSource" : "/CruscottoAuditAtpoWebWeb/jsonATPO/searchCompTecniciProcessoDaAssociare",
					"processing" : false,
					"serverSide" : true,
					"fnServerData": function (sSource, aoData, fnCallback) {
			        	  
			              var idProcesso = jQuery("#idProcesso").val();
			              aoData.push({ "name": "idProcesso", "value": idProcesso });
					...
					"aoColumns" : [
							{
								...
								"mRender" : function(data, type, row) {
									...
									content += '<input type="checkbox"  name="ids" id='+row.id+' class="md-check chkCompTecProcessoAl" value="'+row.id+'">';
		...
		var tableWrapper = $('#addCompTecProcessoTable_wrapper');
