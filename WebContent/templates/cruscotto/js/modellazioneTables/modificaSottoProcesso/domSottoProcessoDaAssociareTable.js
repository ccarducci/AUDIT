var DomSottoProcessoDaAssociareTable = function() {

	var initTable1 = function() {
		
		
		var table = $('#addDominiSottoProcessoTable');
		var oTable = table
				.dataTable( {
					...
					"sAjaxSource" : contextApp + "/jsonATPO/searchDominiSottoProcessoDaAssociare",
					"processing" : false,
					"serverSide" : true,
					...
					"aoColumns" : [
							{
								...
								"mRender" : function(data, type, row) {
									...
									content += '<input type="checkbox"  name="ids" id="domAl_'+row.id+'" class="md-check chkDomSottoProcessoAl" value="'+row.id+'">';
									content += '<label for="domAl_'+row.id+'">';
									content += '<span></span>';
		...
		var tableWrapper = $('#addDominiSottoProcessoTable_wrapper');
