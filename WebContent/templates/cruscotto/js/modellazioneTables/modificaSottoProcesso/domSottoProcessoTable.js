...
var DomSottoProcessoTable = function() {

	var initTable1 = function() {
		
		
		var table = $('#dominiSottoProcessoTable');
		var oTable = table
				.dataTable( {
					...
					"sAjaxSource" : contextApp + "/jsonATPO/searchDominiSottoProcesso",
					"processing" : false,
					"serverSide" : true,
					...
					"aoColumns" : [
							{
								...
								"mRender" : function(data, type, row) {
									...
									content += '<input type="checkbox"  name="ids" id="dom_'+row.idAssociazione+'" class="md-check chkDomSottoProcesso" value="'+row.idAssociazione+'">';
									content += '<label for="dom_'+row.idAssociazione+'">';
									content += '<span></span>';
		...
		var tableWrapper = $('#dominiSottoProcessoTable_wrapper');
