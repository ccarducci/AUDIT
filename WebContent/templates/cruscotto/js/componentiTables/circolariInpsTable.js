var CircolartiInpsTable = function () {

    var initTable1 = function () {
        
    	var table = $('#circolariInpsTable');
        ...
        var oTable = table.dataTable({
            ...
            "sAjaxSource" : "/CruscottoAuditAtpoWebWeb/json/getCircolariInpsTable",
			"processing" : false,
			"serverSide" : true,
			"fnServerData": function (sSource, aoData, fnCallback) {
				...
				var isStorico = false;
				if(document.getElementById('getStoricoNormativa') != null && document.getElementById('getStoricoNormativa') != undefined){
					if(document.getElementById('getStoricoNormativa').checked){
						isStorico = true;
					}
				}
			...
			"aoColumns" : [
					{
						...
						"mRender" : function(data, type, row) {
							...
							content += '<input type="checkbox"  name="ids" id="checkboxTable_'
									+ row.idCircolariInps
									+ '" class="md-check chkCircolariInps" value="'
									+ row.idCircolariInps + '">';
							content += '<label for="checkboxTable_' + row.idCircolariInps + '">';
							content += '<span></span>';
        ...
        var tableWrapper = $('#circolariInpsTable_wrapper'); // datatable creates the table wrapper by adding with id {your_table_jd}_wrapper
